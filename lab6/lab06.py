    ...
    while (True):
        ...
        try:
            frame = cv2.aruco.drawAxis(
                frame, intrinsic, distortion, rvec, tvec, 5)
            tvec_str = "x=%4.0f y=%4.0f z=%4.0f" % (
                tvec[0][0][0], tvec[0][0][1], tvec[0][0][2])
            rvec_str = "x=%4.0f y=%4.0f z=%4.0f" % (
                rvec[0][0][0], rvec[0][0][1], rvec[0][0][2])
...
#             if y_update < -maxSpeed:
#                 y_update = -maxSpeed
#             # cv2.putText(frame, y_update, (20, 460),
#             #             cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2, cv2.LINE_AA)
