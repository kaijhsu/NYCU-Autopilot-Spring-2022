...
def main():
    ...
    time.sleep(10)
    global isFliying
    fs = cv2.FileStorage("camera calibration.xml", cv2.FILE_STORAGE_READ)
    ...
    while (True):
        ...
        yaw_update_deg = 0
        try:
            frame = cv2.aruco.drawAxis(frame, intrinsic, distortion, rvec, tvec, 5)
            tvec_str = "x=%4.0f y=%4.0f z=%4.0f" % (tvec[0][0][0], tvec[0][0][1], tvec[0][0][2])
            rvec_str = "x=%4.0f y=%4.0f z=%4.0f" % (rvec[0][0][0], rvec[0][0][1], rvec[0][0][2])
            ...
            yaw_update_deg = math.degrees(rvec[0][0][2])
        ...
        key = cv2.waitKey(1)
        if key == -1:
            drone.send_rc_control(0, int(z_update), int(y_update), int(yaw_update_deg))
            print("no input")
        else:
            keyboard(drone, key)
